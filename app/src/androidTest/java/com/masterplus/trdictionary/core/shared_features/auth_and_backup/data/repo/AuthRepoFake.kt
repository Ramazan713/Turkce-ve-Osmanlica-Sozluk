...
class AuthRepoFake: AuthRepo {
    ...
    var returnedUserFlow: MutableSharedFlow<User?> = MutableSharedFlow(replay = 1)
    var returnedIsLogin:  Boolean = true


    override suspend fun signInWithEmail(email: String, password: String): Resource<User> {
        return returnedSignInWithEmailResponse.also {
            if(it is Resource.Success){
                returnedUserFlow.emit(it.data)
            }
        }
    }
